...
**Found by Copilot round 1 on `opensoft/openRepoTools#121`, and every one of them taken:**

- **A supervisor sent `SIGTERM` left the lane wedged.** With only an `EXIT` trap, a `TERM` to the pane root ended the process with the intent still `starting` — the one state nothing recovers from by itself, because `--supervise` refuses to launch from it and the next `/ctx` refuses too (`--expect 'none|ready|failed'` is what stops an ordinary clear superseding an operation in flight). **A trap alone would not have fired:** bash defers a trap until the foreground command returns, and this supervisor's foreground command is an interactive Claude. Measured — a shell running `sleep 20` in the foreground did not reach its `TERM` trap for twenty seconds; backgrounded and `wait`ed, it ran in the same second. So the child is backgrounded and `wait`ed, and the two things a background job loses are put back: its **stdin** (job control off redirects an async command's stdin from `/dev/null` — fd 9 holds the pane's own, and the child gets `<&9 9<&-`) and its **`INT`/`QUIT`** (bash sets both to `SIG_IGN` for an async command and SIG_IGN survives `exec`, so Claude would have come up with Ctrl-C dead — measured `SigIgn …0006` without the reset, `…0000` with). `TERM`/`HUP` now mark the intent `failed`, fenced, naming the signal and whether the child was still running, and kill nothing.
- **The intent writer walked past an unknown schema.** The read side fails closed with `UNKNOWN-SCHEMA`; the write side copied the fields it knew forward, dropped the rest and stamped `schema: 1`, which is the read-side protection undone by the other half of one command. Refused now, inside the lock, with 2 rather than 7 — 7 is *another act got there first*, this is *this copy of the tooling must not touch this file*.
- **`lane-start` resumed on an `UNKNOWN-SCHEMA` intent.** The fall-through authorised no FRESH session, which is what it was written for — and the resume source waiting for a lane whose `/ctx` had just killed its pane is the transcript that `/ctx` PAUSED. That is `#94` reached down a second road. It refuses now, naming both ways out.
- **The usage block promised an exit code no path returned.** `4 indeterminate` was never reachable: the observer's `return 4` is read by nothing, because it runs in the background and the record is what decides. An operation whose reason begins `INDETERMINATE:` now ends 4 and says *ran past the readiness deadline still alive*. The `failed` transition **stays** — `INDETERMINATE` is the answer for a child that is ALIVE at the deadline, and that branch is reached only once the child has exited, which is a launch that ended unconfirmed and is retryable; leaving it `starting` would wedge the lane in the state the first item above exists to stop.
- **The failure write was not fenced on the generation.** The readiness finalizer always was; the failure write, the signal write and the `INDETERMINATE` note were not, so a superseded supervisor could publish an old verdict over a newer generation's record. All five fenced now, on the generation **claimed** rather than the one on disk at write time — re-reading it would read the newer generation and match it, which is the write the fence exists to refuse.
...
- **§7.5** — amending `add-automatic-context-rollover` so its success condition is readiness-confirmed `RUNNING`. **There is nothing on this branch to amend:** `openspec/changes/` here holds `add-supervised-context-restart` and `interactive-lane-name-repair` and nothing else, and that change exists only as an untracked working copy in the main checkout. Writing a second copy of somebody else's proposal into this branch to tick a box would be worse than the open box. What this change owes it is the outcome to consume, and that exists: readiness-confirmed completion is the restart intent reaching `ready`, readable from any window with `lane-handoff --restart-status --lane <lane>`, and the requirement *Automatic rollover waits for supervised completion* is in this change's spec for that proposal to point at.
