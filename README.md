...
`--no-push`, a `pushed:` that is missing or neither true nor false, or no
parked commit named for that leg, only the workstation that parked it can;
and where `git worktree list` still holds a registration that is no longer a
worktree, a `worktree prune` — after a `worktree unlock` if it is locked,
with any directory it leaves behind moved aside — has to clear it first), a
...
`--no-push` or with a `pushed:` that is missing or unreadable, and a
