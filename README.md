...
`--no-push`, or a `pushed:` that is neither true nor false, only the
...
diverged from the parked commit, a leg parked with `--no-push` or an
unreadable `pushed:`, and a worktree the record does not know, which was
