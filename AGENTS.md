...
4. **`status` READS AND CHANGES NOTHING, and fetches nothing unless told to**:
   ...
   `resume` can do anything — and a LOCKED one is `git worktree unlock
   <path>` first, because prune SKIPS it and the finding says which kind it
   is; never `worktree add --force`, never a delete under `.git/worktrees/`;
   a worktree BEHIND a newer record — behind by more than the WIP commits
